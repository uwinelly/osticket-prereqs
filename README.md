...
Next, we are going to Remote desktop connect into our VM and install a bunch of prerequisites as well as OsTicket making sure we can log into it.
...
which is one of the requirement for OsTicket to work.
...
Following step is download and install VC_redist.x86.exe.
...
We will go ahead to the start menu, search for IIS right click it and then run it as an administrator.
Double click on the "PHP Manager" icon and then click on register new PHP Version.
Next browse to find your PHP Folder and look for your PHP-CGI file and open it click "ok".
...
Next,we will rename the ost-sampleconfig.php. 
From the windows (C:) drive in file manager go to inetpub > wwwroot > osTicket >include > ost-sampleconfig.php.
Right click and rename to "ost-config.php".We will then assign permissions to the ost-config.php by right clicking and going to properties > security tab.> Advanced. Disable inheritance > Remove all inherited permissions from the object.
...
Next we can go back to the osTicket installer in the browser and enter our MYSQL Database as osTicket and click install now
...
Enter your credentials used while setting up osTicket installer previously.
